...
#불러오기
#netflix data
(read_tsv("~/Documents/netflix/files/all-weeks-countries_220516.tsv") -> netflix1)

#영문 -> 한글 변환
(read_csv("./files/netflix_rank1_nametable2.csv",
          skip = 1) |> select(2,3,4) -> netflix_nametable)
#netflix_nametable |> view()

#한국만 데이터셋 분리
(netflix1 |> filter(
  country_iso2 %in% c("KR")) -> netflix2_kor)
...
#2021
netflix4_TV_name |> filter(year == "2021") |> 
  ggplot(aes(week, 
             y = kor_name |> fct_reorder(week),
             color = original)) + #날짜가 x축일 때 y 축 정렬
...
netflix2_kor_tv |> filter(show_title %in% c("Physical: 100")) |> 
  ggplot(aes(x = weekly_rank |> as.factor())) + 
  geom_bar(stat = "count")

netflix2_kor_tv |> filter(show_title %in% c("Physical: 100")) |> 
  ggplot(aes(week, y = weekly_rank)) + 
  geom_point()
